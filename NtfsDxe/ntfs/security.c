...
{
	return (memcmp(guid, zero_guid, sizeof(*zero_guid)) == 0) ? TRUE : FALSE;
}
...
char *ntfs_sid_to_mbs(const SID *sid, char *sid_str, size_t sid_str_size)
{
	...
	if (!sid->identifier_authority.high_part)
		...
		i = snprintf(s, cnt, "0x%lx", (unsigned long long)u);
	if (i < 0 || i >= cnt)
		goto err_out;
	s += i;
	cnt -= i;
	...
	if ((vol->major_ver < 3) || !vol->secure_ni) {
		...
		if (na) {
			...
			na = ntfs_attr_open(ni, AT_STANDARD_INFORMATION,
				AT_UNNAMED, 0);
			if (na) {
				clear_nino_flag(ni, NI_v3_Extensions);
			/*
		...
		if (securid) {
			...
			if (na) {
				res = 0;
				if (!test_nino_flag(ni, NI_v3_Extensions)) {
			/* expand standard information attribute to v3.x */
				...
				set_nino_flag(ni, NI_v3_Extensions);
...
static char *getsecurityattr(ntfs_volume *vol, ntfs_inode *ni)
{
	...
	if (test_nino_flag(ni, NI_v3_Extensions)
		...
		readallsz = 0;
		securattr = (char *) ntfs_attr_readall(ni, AT_SECURITY_DESCRIPTOR,
				AT_UNNAMED, 0, &readallsz);
		if (securattr && !ntfs_valid_descr(securattr, readallsz)) {
			ntfs_log_error("Bad security descriptor for inode %l\n",
				(long long)ni->mft_no);
			free(securattr);
			securattr = (char*)NULL;
		}
	...
	if (!securattr) {
			 ...
			 */
		ntfs_log_error("No security descriptor found for inode %l\n",
				(long long)ni->mft_no);
...
int ntfs_sd_add_everyone(ntfs_inode *ni)
{
	...
	SID *sid;
	int ret, sd_len;
	unsigned char SID_EVERYONE[80] = {
	0x01, 0x00, 0x04, 0x80, 0x14, 0x00, 0x00, 0x00, 0x24, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 
	0x34, 0x00, 0x00, 0x00, 0x01, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x05, 0x20, 0x00, 0x00, 0x00, 
	0x20, 0x02, 0x00, 0x00, 0x01, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x05, 0x20, 0x00, 0x00, 0x00, 
	0x20, 0x02, 0x00, 0x00, 0x02, 0x00, 0x1C, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x03, 0x14, 0x00, 
	 ...
	 */
//- Removed block. actually, on windows 8 the SECURITY_ATTRIBUTE is into SID_EVERYONE buffer
	...
	//sid->identifier_authority.value[5] = 1;	// 5 SID NT_AUTHORITY
	//sd->owner = cpu_to_le32((u8*)sid - (u8*)sd);
	//
	//sid = (SID*)((u8*)sid + sizeof(SID) + 4); 
	//sid->revision = SID_REVISION;
	//sid->sub_authority_count = 2;
	//sid->sub_authority[0] = const_cpu_to_le32(SECURITY_BUILTIN_DOMAIN_RID);	// SECURITY_BUILTIN_DOMAIN_RID
	//sid->sub_authority[1] = const_cpu_to_le32(DOMAIN_ALIAS_RID_ADMINS);	// DOMAIN_ALIAS_RID_ADMINS
	//sid->identifier_authority.value[5] = 1; // 5 SID NT_AUTHORITY
...
//+
	...
	ret = ntfs_attr_add(ni, AT_SECURITY_DESCRIPTOR, AT_UNNAMED, 0, (u8*) SID_EVERYONE, sizeof(SID_EVERYONE));
			...
			ntfs_inode *dir_ni, BOOL fordir)
{
	...
	cached = (struct CACHED_PERMISSIONS*)NULL;
		 ...
		 */
	if (test_nino_flag(dir_ni, NI_v3_Extensions)
			&& dir_ni->security_id) {
		cached = fetch_cache(scx, dir_ni);
		if (cached)
