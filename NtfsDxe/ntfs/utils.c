...
#define LONG_MAX 260
#define isprint(c) (c >= 32 && c <= 127) ? 1 : 0
#include "utils.h"
#include "types.h"
#include "volume.h"
#include "debug.h"
...
ntfschar *ntfs_attr_get_name(ATTR_RECORD *attr)
{
...
int utils_valid_device(const char *name, int force)
{
	...
	if (ntfs_check_if_mounted(name, &mnt_flags)) {
		ntfs_log_perror("Failed to determine whether %s is mounted",
				name);
		if (!force) {
			ntfs_log_perror("Use the force option to ignore this "
					"error.\n");
			return 0;
		}
...
int utils_parse_range(const char *string, s64 *start, s64 *finish, BOOL scale)
{
	...
	if (middle) {
		if (middle[1] == 0) {
			...
			ntfs_log_debug("Range has no end, defaulting to %l.\n", b);
		} else {
...
int utils_cluster_in_use(ntfs_volume *vol, long long lcn)
{
	...
	ntfs_log_debug("cluster = %l, bmplcn = %l, byte = %d, bit = %d, "
...
int utils_mftrec_in_use(ntfs_volume *vol, MFT_REF mref)
{
	...
	ntfs_log_debug("cluster = %l, bmpmref = %l, byte = %d, bit = %d, in use %d\n", mref, bmpmref, byte, bit, buffer[byte] & bit);
		...
		if (in_use) {
			...
			switch (utils_is_metadata(ctx->inode)) {
				...
				default:
					...
					//ntfs_log_error("Error reading inode %l.\n", ctx->mft_num);
					//return -1;
			}
 ...
 *	specifications (%l, %lx, etc). We have to translate them
 *	to %I64.
 */
